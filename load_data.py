...
class CsvDataset(Dataset):
    ...
    def __init__(self, file, need_prepro, pipeline, max_len, mode, d_type):
        ...
        # need preprocessing
        if need_prepro:
            with open(file, 'r', encoding="utf-8", errors='ignore') as f:    # iso8859-1
                ...
                # unsupervised dataset
                    ...
                    # edited by YuchuanFu on 25 June, 单独处理unsup数据，直接进行同义词替换数据增强
                    # version1: 适用于直接输入csv文件，每行为ori, aug
                    data = {'ori': [], 'aug': []}
                    ...
                    data = []
                    for instance in self.get_pred(lines):
                        for proc in pipeline:
                            instance = proc(instance, d_type)
                        data.append(instance)
        ...
        # already preprocessed, 只能在IMDB数据上使用，直接打开现成处理好的数据
        else:
    ...
    def get_pred(self, lines):  # added by YuchuanFu on 28 June
        raise NotImplementedError
...
#             yield (None, line[1], []), (None, line[2], [])  # ko, en
            yield (None, line[0], []), (None, line[1], [])
        
class forexc(CsvDataset):
    labels = ('0', '1')
    def __init__(self, file, need_prepro, pipeline=[], max_len=128, mode='train', d_type='sup'):
        super().__init__(file, need_prepro, pipeline, max_len, mode, d_type)
    ...
    def get_pred(self, lines):  # added by YuchuanFu on 28 June
        for line in itertools.islice(lines, 0, None):
            yield None, line[0], []    # label, text_a, text_b
...
class load_data:
    def __init__(self, cfg):
        ...
        elif cfg.mode == 'pred':   # added by YuchuanFu on 28 June
            self.pred_data_dir = cfg.pred_data_dir
            self.pred_batch_size = cfg.pred_batch_size
            self.shuffle = False    
    ...
    def pred_data_iter(self):  # added by YuchuanFu on 28 June
        pred_dataset = self.TaskDataset(self.pred_data_dir, self.cfg.need_prepro, self.pipeline, self.cfg.max_seq_length, 'pred', 'pred')  # mode, d_type
        pred_data_iter = DataLoader(pred_dataset, batch_size=self.pred_batch_size, shuffle=False)

        return pred_data_iter
